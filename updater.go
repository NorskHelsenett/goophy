...
func NewAutoUpdater(options UpdateOptions) *AutoUpdater {
...
func (u *AutoUpdater) downloadAndApplyUpdate(url, assetName string) error {
	...
	if strings.HasSuffix(assetName, ".tar.gz") || fileExt == ".gz" {
		...
		updateData, err = extractBinaryFromTarGz(bytes.NewReader(bodyBytes), binaryName)
	} else if fileExt == ".zip" {
		// Handle zip archive
		updateData, err = extractBinaryFromZip(bytes.NewReader(bodyBytes), int64(len(bodyBytes)), binaryName)
	} else {
		// Direct binary
